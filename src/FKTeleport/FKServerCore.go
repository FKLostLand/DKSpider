...
func (self *TeleportNode) Server(port ...string) {
	self.reserveAPI()
	self.mode = SERVER
	// 设置端口
	if len(port) > 0 {
...
// 以服务器模式启动
func (self *TeleportNode) server() {
	var err error
retry:
	self.listener, err = net.Listen("tcp", self.port)
	if err != nil {
		Printf("Debug: 监听端口出错: %v", err)
	...
	for self.listener != nil {
		...
		Printf("Debug: 客户端 %v 已连接，但尚未验证身份！", conn.RemoteAddr().String())
...
// 为每个连接开启读写两个协程
func (self *TeleportNode) sGoConn(conn net.Conn) {
...
func (self *TeleportNode) sInitConn(conn *Connect, remoteAddr string) (nodeuid string, usable bool) {
	...
	for i, data := range dataSlice {
		Println("Debug: 收到数据-第1批-解码前: ", string(data))
		...
		if i == 0 {
			Printf("Debug: 收到数据-第1条-NetData: %+v", d)
...
// 服务器读数据
func (self *TeleportNode) sReader(nodeuid string) {
	// 退出时关闭连接，删除连接池中的连接
	defer func() {
...
// 服务器发送数据
func (self *TeleportNode) sWriter(nodeuid string) {
	defer func() {
		self.closeConn(nodeuid, false)
	}()
