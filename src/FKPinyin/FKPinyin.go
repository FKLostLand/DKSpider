...
func CreatePinyin(s string, a Args) [][]string {
	hans := []rune(s)
	var pys [][]string
	for _, r := range hans {
...
func CreateLazyPinyin(s string, a Args) []string {
	a.Heteronym = false
	var pys []string
	for _, v := range CreatePinyin(s, a) {
		pys = append(pys, v[0])
	}
	return pys
}
...
// 横杠连接LazyPinyin的返回值.
func CreateSlugLazyPinyin(s string, a Args) string {
	separator := a.Separator
	return strings.Join(CreateLazyPinyin(s, a), separator)
}
...
func SortInitials(strs []string) {
	...
	for i := 0; i < l; i++ {
		for ii, py := range CreatePinyin(strs[i], a) {
			if len(py) == 0 {
