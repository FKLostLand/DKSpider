...
func init() {
	Retail_ZolPhine_Spider.RegisterToSpiderSpecies()
}

var Retail_ZolPhine_Spider = &FKSpider.Spider{
	Name:        "论坛站_中关村手机",
	Description: "手机论坛，抓取用户帖子 [http://bbs.zol.com.cn/sjbbs/]",
	EnableCookie: false,
		...
		Trunk: map[string]* FKSpider.Rule{
			...
			"生成请求": {
				AidFunc: func(ctx *FKSpider.Context, aid map[string]interface{}) interface{} {
					header := http.Header{}
					header.Add("X-AUSERNAME", "frankie.w")
					header.Add("X-ASEN", "YOU MAKE ME A SAD PANDA.")
